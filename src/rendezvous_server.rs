...
impl RendezvousServer {
    ...
    pub async fn start(host: &str, port: i32, serial: i32, key: &str, rmem: usize) -> ResultType<()> {
        ...
        log::info!("Listening on tcp/udp {}:{}", host, port);
        log::info!("Listening on tcp {}:{}, extra port for NAT test", host, nat_port);
        log::info!("Listening on websocket {}:{}", host, ws_port);
        let mut socket = create_udp_listener(host, port, rmem).await?;
        ...
        let mut listener = create_tcp_listener(host, port).await?;
        let mut listener2 = create_tcp_listener(host, nat_port).await?;
        let mut listener3 = create_tcp_listener(host, ws_port).await?;
            ...
            loop {
                    ...
                    .await
                {
                    LoopFailure::UdpSocket => {
                        drop(socket);
                        socket = create_udp_listener(host, port, rmem).await?;
                    }
                    LoopFailure::Listener => {
                        drop(listener);
                        listener = create_tcp_listener(host, port).await?;
                    }
                    LoopFailure::Listener2 => {
                        drop(listener2);
                        listener2 = create_tcp_listener(host, nat_port).await?;
                    }
                    LoopFailure::Listener3 => {
                        drop(listener3);
                        listener3 = create_tcp_listener(host, ws_port).await?;
                    }
                }
            }
        };
...
async fn create_udp_listener(host: &str, port: i32, rmem: usize) -> ResultType<FramedSocket> {
    if hbb_common::is_ipv6_str(host)
    {
        let addripv6: Ipv6Addr = host.parse().expect("parse failed");
        let addr = SocketAddr::new(IpAddr::V6(addripv6), port as _);
        if let Ok(s) = FramedSocket::new_reuse(&addr, true, rmem).await {
            log::info!("listen on udp {:?}", s.local_addr());
            return Ok(s);
        }
    }
    let addripv4: Ipv4Addr = host.parse().expect("parse failed");
    let addr = SocketAddr::new(IpAddr::V4(addripv4), port as _);
...
#[inline]
async fn create_tcp_listener(host: &str, port: i32) -> ResultType<TcpListener> {
    let s = listen_any(host, port as _, true).await?;
    log::info!("listen on tcp {:?}", s.local_addr());
    Ok(s)
}
